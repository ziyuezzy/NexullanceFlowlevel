...
sys.path.append(os.path.abspath(os.path.join(os.getcwd(), '../../..')))
from topologies.Polarfly import Polarflytopo
...
def main():
    ...
    with open(filename, 'w', newline='') as csvfile:
        ...
        for V, D in configs:
            ...
            max_core_link_load = np.max(core_link_flows)/Cap_core
            max_access_link_load = np.max(access_link_flows)/Cap_access
            nexu = Nexullance_MP(_network.nx_graph, APST4, gl.convert_M_EPs_to_M_R(M_EPs, V, EPR), Cap_core, 0, False)
            nexu.init_model()
            Lcore_NEXU, _ = nexu.solve()
