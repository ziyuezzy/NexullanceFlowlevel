...
def main():
    ...
    for traffic_name, baseline_M in Demand_matrices.items():
        ...
        # apply ECMP_ASP and Nexullance_MP_APST4(original) on each of the traffic demand matrix
        ECMP_ASP_phis = {key: [] for key in perturbated_Ms}
        Nexullance_MP_APST_original_phis = {key: [] for key in perturbated_Ms}
        # calculate the routing table of Nexullance_MP_APST4 for 'original'
        ...
        for rate, M_list in perturbated_Ms.items():
            for M in M_list:
                ...
                # calculate phi
                Nexullance_MP_APST_original_phis[rate].append(gl.network_total_throughput(M, max(core_link_flows)/Cap_core, max(access_link_flows)/Cap_access)/(V*EPR))
            ...
            for rate, M_list in perturbated_Ms.items():
                for repetition_id in range(len(M_list)):
                    csvwriter.writerow([ V, D, rate, repetition_id, Nexullance_MP_APST_phis[rate][repetition_id], 
                        Nexullance_MP_APST_original_phis[rate][repetition_id], ECMP_ASP_phis[rate][repetition_id] ])
                    csvfile.flush()
