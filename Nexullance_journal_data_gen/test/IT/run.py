...
def main():
    ...
    for topo_name, configs in Topo_config.items():
        ...
        with open(filename, 'w', newline='') as csvfile:
            ...
            csvwriter.writerow(['V', 'D', 'EPR', 'traffic', 'phi', 'max_core_load'])
            for V, D in configs:
                EPR = (D+1)//2
                ...
                for traffic_name, M_EPs in Demand_matrices.items():
                    result_phi, max_core_load = exp_container.run_nexullance_IT(M_EPs, traffic_name, False)
                    if result_phi:
                        csvwriter.writerow([V, D, EPR, traffic_name, result_phi, max_core_load])
                        csvfile.flush()
