...
csv_cols = [
    'V', 'D', 'M_id', 
    'phi_ECMP_ASP', 'phi_IT', 'phi_MD_IT', 'phi_diff_IT',
    'time_IT', 'time_MD_IT', 'time_diff_IT',
    'attempts_IT', 'attempts_MD_IT', 'attempts_diff_IT'
]

_num_samples_sweep = [1, 2, 4, 8, 16, 32, 64, 128, 256]
_repetitions = 5  # number of repetitions for each experiment
    ...
    for sampling_method in ["sent", "enroute"]:
        ...
        for bench, problem_size, args_func in benchmarks:
            ...
            for _num_samples in _num_samples_sweep:
                ...
                MD_IT_result = exp_container.run_and_profile_MD_nexullance_IT(M_EPs_s, 
                                            [1/_num_samples for _ in range(_num_samples)], repetitions=_repetitions, auto_scaling=True)
                ...
                diff_nexu_result = exp_container.run_and_profile_diff_nexullance_IT(M_EPs_s, repetitions=_repetitions, auto_scaling=True)

                filename = f'{topo_name}_{V}_{D}_{_num_samples}_{sampling_method}_samples_{bench}.csv'
                ...
                with open(filename, 'w', newline='') as csvfile:
                    ...
                    for i in range(len(M_EPs_s)):
                        csvwriter.writerow([V, D, i, ECMP_ASP_phis[i], IT_results[i]["ave_phi"],
                                            MD_IT_result["ave_phi"][i], diff_nexu_result["ave_phi"][i],
                                            IT_results[i]["ave_time[s]"], MD_IT_result["ave_time[s]"], diff_nexu_result["ave_time[s]"][i],
                                            IT_results[i]["ave_num_attempts"], MD_IT_result["ave_num_attempts"], diff_nexu_result["ave_num_attempts"][i]]
                        )
                        csvfile.flush()
